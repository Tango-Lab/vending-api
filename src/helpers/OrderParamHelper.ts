...
export interface OrderProductsRequestParam {
  id: string;
  quantity: number;
  slotNo: string;
}
...
export type ValidationRulesMap = {
  [K in keyof OrderRequestParams]: ValidationRules;
};
...
export class OrderParamsChecker {
  #requestBody!: OrderRequestParams;
  ...
  constructor(request: Request<any, any, OrderRequestParams>, param: ValidationRulesMap) {
    if (!request.body) {
   ...
   */
  getParams(): OrderRequestParams {
    const validatedParams: Partial<OrderRequestParams> = {};
    const paramKeys = Object.keys(this.#VALIDATION_RULES) as (keyof OrderRequestParams)[];
    ...
    return validatedParams as OrderRequestParams;
  }
   ...
   */
  #validate() {
    const { serialNo, machine, products } = this.#requestBody;
    ...
    this.#validateArray(products, 'products', this.#VALIDATION_RULES.products);
  }
  ...
  #validateArray(array: any[], fieldName: string, rules: ValidationRules) {
    if (rules.isArray && !Array.isArray(array)) {
      throw new MissingParamError(fieldName);
    }
    ...
    if (rules.maxLength && array.length > rules.maxLength) {
      throw new BadRequestError(
        `'${fieldName}' array must not contain more than ${rules.maxLength} items`,
        ErrorCode.OrderProductLimitExceeded,
      );
    }
    ...
    if (rules.itemRules) {
      array.forEach((item, index) => {
        for (const [key, itemRules] of Object.entries(rules.itemRules as ValidationRules)) {
          this.#validateField(item[key], `products[${index}].${key}`, itemRules as ValidationRules);
        }
      });
    }
  }
